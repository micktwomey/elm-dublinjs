...
port fetchTextures =
  loadTexture "/examples/texture/thwomp_face.jpg" `Task.andThen` \faceTexture ->
  loadTexture "/examples/texture/thwomp_side.jpg" `Task.andThen` \sideTexture ->
...
perspective (w',h') (x',y') =
    let w = toFloat w'
        ...
        _ = Debug.watch "window w,h" (w', h')
        _ = Debug.watch "mouse x,y" (x', y')
        _ = Debug.watch "eye" (eyeX, eyeY)
    in
